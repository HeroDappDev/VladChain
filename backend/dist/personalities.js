...
exports.personalitiesRouter = void 0;
const express_1 = __importDefault(require("express"));
exports.personalitiesRouter = express_1.default.Router();
...
function generateCreativeVladchainResponse(validator, userMessage) {
    ...
    const responses = fallbackResponses[validator] || [
...
function checkRepetition(validatorId, newResponse) {
...
function updateSessionMemory(validatorId, response) {
    ...
    if (session.lastOpenings.length > 5)
        session.lastOpenings.shift();
    if (session.lastResponses.length > 5)
        session.lastResponses.shift();
}
exports.personalitiesRouter.post('/:validator', async (req, res) => {
    const { validator } = req.params;
    const { message: userMessage, conversationHistory = [] } = req.body;
    const val = validators[validator.toLowerCase()];
    if (!val) {
...
exports.personalitiesRouter.post('/:validator/clear-session', (req, res) => {
    const { validator } = req.params;
