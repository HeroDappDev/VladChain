...
exports.generateSampleGIPs = generateSampleGIPs;
...
This protocol will ensure VladChain's RWA reserves survive the failure of any one custodian.`,
        category: gip_types_1.GIPCategory.SECURITY,
        priority: gip_types_1.GIPPriority.CRITICAL,
...
This pool will keep the RWA layer surprising, adventurous, and truly alive.`,
        category: gip_types_1.GIPCategory.GOVERNANCE,
        priority: gip_types_1.GIPPriority.LOW,
...
async function generateSampleGIPs() {
    console.log('Generating sample GIPs...');
    for (const gipData of sampleGIPs) {
        try {
            const gip = await gip_system_1.gipSystem.createGIP(gipData.author, gipData.title, gipData.summary, gipData.fullProposal, gipData.category, gipData.priority, gipData.tags);
            ...
            if (gipData.debateThread && gipData.debateThread.length > 0) {
                ...
                const updatedGip = gip_system_1.gipSystem.getGIP(gip.id);
                if (updatedGip) {
                    ...
                    updatedGip.status = 'debating'; // Mark as debating since it has debate
                }
            ...
            if (!gipData.debateThread || gipData.debateThread.length === 0) {
                if (Math.random() > 0.3) {
                    await gip_system_1.gipSystem.startDebate(gip.id);
