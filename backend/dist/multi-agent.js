...
exports.getAllAgentResponses = getAllAgentResponses;
exports.getRandomAgentResponse = getRandomAgentResponse;
exports.clearAgentHistory = clearAgentHistory;
exports.clearAllAgentHistory = clearAllAgentHistory;
...
async function getAgentResponse(agentId, userMessage) {
    const agent = exports.agents[agentId];
    if (!agent) {
    ...
    try {
        ...
        const response = await (0, claude_1.claudeChatCompletion)(agent.systemPrompt, userMessage, agent.model);
...
async function getAllAgentResponses(userMessage) {
    const responses = [];
    // Get responses from all agents in parallel
    const promises = Object.keys(exports.agents).map(async (agentId) => {
        try {
            const response = await getAgentResponse(agentId, userMessage);
            return {
                agentId,
                name: exports.agents[agentId].name,
                response
            };
        }
        catch (error) {
            ...
            return {
                agentId,
                name: exports.agents[agentId].name,
                response: `[${exports.agents[agentId].name}] Sorry, I'm having trouble connecting to my AI brain right now. Please check the API configuration.`
            };
        }
    });
...
async function getRandomAgentResponse(userMessage) {
    const agentIds = Object.keys(exports.agents);
    ...
    try {
        ...
        return {
            ...
            name: exports.agents[randomAgentId].name,
            response
        };
    }
    catch (error) {
        ...
        return {
            agentId: randomAgentId,
            name: exports.agents[randomAgentId].name,
            response: `[${exports.agents[randomAgentId].name}] Sorry, I'm having trouble connecting to my AI brain right now. Please check the API configuration.`
        };
    }
}
...
function clearAllAgentHistory() {
    Object.keys(exports.agents).forEach(agentId => {
