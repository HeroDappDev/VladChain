...
exports.chatlogRouter = void 0;
exports.addEventChatToLog = addEventChatToLog;
const express_1 = __importDefault(require("express"));
// Removed claudeChatCompletion import - no longer using API calls for automatic messages
const database_1 = require("./database");
exports.chatlogRouter = express_1.default.Router();
// GET endpoint to retrieve chat log
exports.chatlogRouter.get('/', (req, res) => {
    const sessionId = req.query.session_id;
    const messages = database_1.db.getChatMessages(100, sessionId);
    res.json(messages);
});
// POST endpoint to add a chat message
exports.chatlogRouter.post('/', (req, res) => {
    ...
    const id = database_1.db.addChatMessage(message);
    res.json({ success: true, id, message });
});
// DELETE endpoint to clear chat messages
exports.chatlogRouter.delete('/', (req, res) => {
    const sessionId = req.query.session_id;
    database_1.db.clearChatMessages(sessionId);
    res.json({ success: true });
});
// Initialize with default messages if database is empty
const initializeDefaultMessages = () => {
    const existingMessages = database_1.db.getChatMessages(10);
    if (existingMessages.length === 0) {
        ...
        defaultMessages.forEach(msg => database_1.db.addChatMessage(msg));
    }
};
...
function shouldCommentOnBlock(blockHeight) {
    return blockHeight % 10 === 0;
}
// Function to get block messages for a specific block height
function getBlockMessages(blockHeight) {
    return BLOCK_MESSAGES[blockHeight] || null;
}
...
function addEventChatToLog(type, main, details = {}) {
    const now = Date.now();
    switch (type) {
        case 'block': {
                ...
                if (blockMessages) {
                    ...
                    blockMessages.forEach((msg, index) => {
                        setTimeout(() => {
                            ...
                            database_1.db.addChatMessage(message);
            ...
            if (shouldStartNewConversation()) {
                ...
                database_1.db.addChatMessage(initiatorMessage);
                ...
                setTimeout(() => {
                    ...
                    database_1.db.addChatMessage(responderMessage);
                    conversationDepth++;
                ...
                const messages = database_1.db.getChatMessages(1);
                ...
                setTimeout(() => {
                    ...
                    database_1.db.addChatMessage(responseMessage);
                    ...
                    if (Math.random() < 0.05 && conversationDepth < 1) {
                        setTimeout(() => {
                            ...
                            database_1.db.addChatMessage(followUpMessage);
        ...
        case 'epoch': {
            ...
            database_1.db.addChatMessage(epochMessage);
            ...
            setTimeout(() => {
                ...
                database_1.db.addChatMessage(aliceMessage);
        ...
        case 'faucet': {
            ...
            database_1.db.addChatMessage(faucetMessage);
            ...
            setTimeout(() => {
                ...
                database_1.db.addChatMessage(ayraMessage);
        ...
        case 'send': {
            ...
            database_1.db.addChatMessage(sendMessage);
            break;
        }
        case 'account': {
            ...
            database_1.db.addChatMessage(accountMessage);
            break;
        }
    }
}
